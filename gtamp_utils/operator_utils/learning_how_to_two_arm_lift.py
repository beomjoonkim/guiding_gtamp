...
def compute_two_arm_grasp():
    ...
    for yaw in yaw_list:
        ...
        right_g_config = get_manipulator(robot).FindIKSolution(rightarm_tool,
                                                               0)  # finds the solution without checking collision
        ...
        left_g_config = get_manipulator(robot).FindIKSolution(leftarm_tool,
                                                              0)  # finds the solution without checking collision
    ...
    new_config = manip.GetArmDOFValues();
    new_config[1] = new_config[1] * 0;
    set_config(robot, new_config, manip.GetArmIndices())
    manip = robot.GetManipulator('leftarm');
    new_config = manip.GetArmDOFValues();
    new_config[1] = new_config[1] * 0;
    set_config(robot, new_config, manip.GetArmIndices())
...
def translate_ee():
    ...
    ee_wrt_o = np.eye(4);
    ee_wrt_o[0, 3] = 0.1  # desired ee location
    ...
    ee_wrt_w = np.eye(4);
    ee_wrt_w[:3, :3] = ee_rot_wrt_w;
    ee_wrt_w[:, 3] = ee_pos_wrt_w
